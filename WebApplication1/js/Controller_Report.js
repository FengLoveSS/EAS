app.controller('Report', function ($scope, $http) {
    ...
    var getMaxPage = function () {//获得最大的页码数
        $http.get("../asp.net/Server.asmx/GetReportMaxPage?CNO=" + $scope.CNO + "&CName=" + $scope.CName + "&TName=" + $scope.TName + "&DNO=" + $scope.Department).success(function (result) {
    ...
    $scope.select = function (page) {//取数据
        $scope.dataset = null;
        $scope.run = true;
        $http.get("../asp.net/Server.asmx/GetReportList?" + "page=" + page + "&DNO=" + $scope.Department + "&TName=" + $scope.TName + "&CNO=" + $scope.CNO + "&CName=" + $scope.CName).success(function (result) {
